...
kNearest = 4
converterJar = "methodFeatureExtractor.jar"
####
...
# when server starts, first load the existing vectors in place
loadData()
...
def equalJaccardNearest():
    ...
    proc = Popen(['java', '-jar', converterJar], stdout=PIPE, stdin=PIPE, stderr=STDOUT)
    output = proc.communicate(input=request.form['method'].encode())
