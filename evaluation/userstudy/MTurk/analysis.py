...
batch1_folder='batch1'
batch2_folder='batch2'
batch1_filter_users = [1,3]
batch2_filter_users = [1]
name = "MTurk"
...
def main():
    vote_batch1_file = os.path.join(batch1_folder,'codereco_vote.csv')
    user_batch1_file = os.path.join(batch1_folder, 'auth_user.csv')
    weight_batch1_file = os.path.join(batch1_folder, 'codereco_customweight.csv')
    submission_batch1_file = os.path.join(batch1_folder, 'codereco_submission.csv')
    cutcopy_batch1_file = os.path.join(batch1_folder, 'codereco_cutcopy.csv')

    votes_batch1 = pd.read_csv(vote_batch1_file)
    users_batch1 = pd.read_csv(user_batch1_file)
    weights_batch1 = pd.read_csv(weight_batch1_file)
    submissions_batch1 = pd.read_csv(submission_batch1_file)
    cutcopy_batch1 = pd.read_csv(cutcopy_batch1_file)
    
    vote_batch2_file = os.path.join(batch2_folder,'codereco_vote.csv')
    user_batch2_file = os.path.join(batch2_folder, 'auth_user.csv')
    weight_batch2_file = os.path.join(batch2_folder, 'codereco_customweight.csv')
    submission_batch2_file = os.path.join(batch2_folder, 'codereco_submission.csv')
    cutcopy_batch2_file = os.path.join(batch2_folder, 'codereco_cutcopy.csv')

    votes_batch2 = pd.read_csv(vote_batch2_file)
    users_batch2 = pd.read_csv(user_batch2_file)
    weights_batch2 = pd.read_csv(weight_batch2_file)
    submissions_batch2 = pd.read_csv(submission_batch2_file)
    cutcopy_batch2 = pd.read_csv(cutcopy_batch2_file)

    # filter test users
    study_batch1 = votes_batch1[~votes_batch1.user_id.isin(batch1_filter_users)].copy()
    study_batch2 = votes_batch2[~votes_batch2.user_id.isin(batch1_filter_users)].copy()
    
    votes_batch1 = study_batch1.merge(users_batch1, left_on='user_id', right_on='id')[['question','source','rank','rating','code','username','voteid']]
    votes_batch2 = study_batch2.merge(users_batch2, left_on='user_id', right_on='id')[['question','source','rank','rating','code','username','voteid']]
     
    weights_batch1 = weights_batch1[~weights_batch1.user_id.isin(batch1_filter_users)]\
                    .merge(users_batch1, left_on='user_id', right_on='id')[['signature', 'structure', 'concepts', 'language','username','code','vote_id']]
    weights_batch2 = weights_batch2[~weights_batch2.user_id.isin(batch2_filter_users)]\
                    .merge(users_batch2, left_on='user_id', right_on='id')[['signature', 'structure', 'concepts', 'language','username','code','vote_id']]
    
    
    submissions_batch1 = submissions_batch1[~submissions_batch1.user_id.isin(batch1_filter_users)]\
                    .merge(users_batch1, left_on='user_id',right_on='id')[['question','code','username']]
    submissions_batch2 = submissions_batch2[~submissions_batch2.user_id.isin(batch2_filter_users)]\
                    .merge(users_batch2, left_on='user_id',right_on='id')[['question','code','username']]
    
    copies_batch1 = cutcopy_batch1[~cutcopy_batch1.user_id.isin(batch1_filter_users)]\
                    .merge(users_batch1, left_on='user_id',right_on='id')[['question','source','rank','username']]
    copies_batch2 = cutcopy_batch2[~cutcopy_batch2.user_id.isin(batch2_filter_users)]\
                    .merge(users_batch2, left_on='user_id',right_on='id')[['question','source','rank','username']]
    
    
    # Merging
    votes = pd.concat([votes_batch1, votes_batch2], ignore_index=True)
    weights = pd.concat([weights_batch1, weights_batch2], ignore_index=True)
    submissions = pd.concat([submissions_batch1, submissions_batch2], ignore_index=True)
    copies = pd.concat([copies_batch1, copies_batch2], ignore_index=True)
